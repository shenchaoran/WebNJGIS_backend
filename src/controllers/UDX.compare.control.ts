...
export const compare = (UDXs: Array<UDXCfg>): Promise<any> => {
    const types = _
        .chain(UDXs)
        .map(udxcfg => udxcfg.schema$.externalName)
        .value();
    const uniqTypes = _.uniq(types);    
    if (uniqTypes.length === 1) {
        let promise;
        if(uniqTypes[0] === ExternalName[ExternalName.TABLE_RAW]) {
            promise = compareRAWTable(UDXs);
        }
        else if(uniqTypes[0] === ExternalName[ExternalName.ASCII_GRID_RAW]) {
            promise = compareRAWAscii(UDXs);
        }
        else if(uniqTypes[0] === ExternalName[ExternalName.SHAPEFILE_RAW]) {
