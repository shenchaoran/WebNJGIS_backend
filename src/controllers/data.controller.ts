...
export default class DataCtrl {
	...
	static convert2Tree = (user, docs: Array<any>): Promise<any> => {
	 ...
	 */
	static pushData = (_id: string): Promise<any> => {
		let doc = undefined;
			...
			geoDataDB
				...
				.then(response => {
					...
					if (response.res === 'suc') {
						...
						return reject(
							new Error('post to model server failed!')
						);
					}
				})
				.then(rst => {
	 ...
	 */
	static pullData = (output): Promise<any> => {
		let extName = undefined;
		// let dataType: GeoDataType;
		const oid = new ObjectID();
		const oidStr = oid.toHexString();
		const url = APIModel.getAPIUrl('download-geo-data', {
			id: output.DataId
		});
	 ...
	 * deprecated
	 */
	static compareUDX = (req: Request, res: Response, next: NextFunction) => {
